/**
 * Browser-Compatible Dynamic Pricing Service
 * Version of DynamicPricingService that works in the browser
 * Uses fetch API and localStorage instead of Node.js modules
 */
...
class BrowserDynamicPricingService {
  ...
  async getTokenPrice(symbol: string): Promise<TokenPrice> {
    ...
    if (!address) {
      logger.warn('BrowserDynamicPricingService', `No address mapping for token: ${symbol}`);
    ...
    try {
      ...
      logger.error('BrowserDynamicPricingService', `Failed to fetch price for ${symbol}:`, error);
  ...
  async calculateDynamicInputAmount(symbol: string, targetUSD: number, decimals: number): Promise<{
    ...
    // Convert to wei/smallest unit using BigInt to avoid scientific notation
    const tokenAmountScaled = Math.floor(tokenAmount * (10 ** decimals));
    const inputAmount = BigInt(tokenAmountScaled).toString();
    
    logger.info('BrowserDynamicPricingService', 
  ...
  async updateAllPrices(): Promise<{
    ...
    if (this.isUpdating) {
      logger.info('BrowserDynamicPricingService', 'Price update already in progress, skipping');
      return this.getLastUpdateResults();
    }

    this.isUpdating = true;
    logger.info('BrowserDynamicPricingService', 'Starting daily price update...');
    ...
    try {
      ...
      for (const symbol of tokens) {
        try {
          logger.debug('BrowserDynamicPricingService', `Updating price for ${symbol}...`);
          ...
          if (symbol !== tokens[tokens.length - 1]) { // Don't wait after last token
            logger.debug('BrowserDynamicPricingService', 'Rate limiting: waiting 1 second...');
            await new Promise(resolve => setTimeout(resolve, 1000));
          }
        } catch (error) {
          logger.error('BrowserDynamicPricingService', `Failed to update ${symbol}:`, error);
  ...
  private async fetchTokenPriceWithRetry(symbol: string, retryAttempt: number = 0): Promise<TokenPrice> {
    ...
    if (coinGeckoId) {
      try {
        ...
        logger.warn('BrowserDynamicPricingService', `Simple price API failed for ${symbol}, trying token price API`);
      }
    }
    ...
    if (address) {
      try {
        ...
        if (retryAttempt < this.retryDelays.length) {
          ...
          logger.warn('BrowserDynamicPricingService', `Retry ${retryAttempt + 1} for ${symbol} in ${delay}ms`);
  ...
  private async fetchFromSimplePriceAPI(symbol: string, coinGeckoId: string): Promise<TokenPrice> {
    ...
    logger.debug('BrowserDynamicPricingService', `Fetching simple price for ${symbol}: ${coinGeckoId}`);
    ...
    logger.debug('BrowserDynamicPricingService', `Cached ${symbol}: $${price.price_usd} (simple API)`);
    return price;
  }
  ...
  private async fetchFromTokenPriceAPI(symbol: string, address: string): Promise<TokenPrice> {
    ...
    logger.debug('BrowserDynamicPricingService', `Fetching token price for ${symbol}: ${address}`);
    ...
    logger.debug('BrowserDynamicPricingService', `Cached ${symbol}: $${price.price_usd} (token API)`);
    return price;
  }
  ...
  private loadCacheFromStorage(): void {
    try {
      const cacheData = localStorage.getItem('dynamicPricing_cache');
      const cacheTime = localStorage.getItem('dynamicPricing_cacheTime');
      
      if (cacheData && cacheTime) {
        const parsedCache = JSON.parse(cacheData);
        this.lastCacheTime = parseInt(cacheTime);
        
        // Reconstruct Map from stored data
        for (const [address, price] of Object.entries(parsedCache)) {
          this.priceCache.set(address, price as TokenPrice);
        }
        
        logger.debug('BrowserDynamicPricingService', `Loaded ${this.priceCache.size} prices from localStorage`);
      }
    } catch (error) {
      logger.warn('BrowserDynamicPricingService', 'Failed to load cache from localStorage:', error);
    }
  }
  ...
  private saveCacheToStorage(): void {
    try {
      const cacheData = Object.fromEntries(this.priceCache.entries());
      localStorage.setItem('dynamicPricing_cache', JSON.stringify(cacheData));
      localStorage.setItem('dynamicPricing_cacheTime', this.lastCacheTime.toString());
      
      logger.debug('BrowserDynamicPricingService', `Saved ${this.priceCache.size} prices to localStorage`);
    } catch (error) {
      logger.warn('BrowserDynamicPricingService', 'Failed to save cache to localStorage:', error);
    }
  }
  ...
  private async ensurePricesLoaded(): Promise<void> {
    ...
    if (now - this.lastCacheTime > this.cacheExpiry || this.priceCache.size === 0) {
      logger.info('BrowserDynamicPricingService', 'Price cache expired or empty, triggering update...');
  ...
  private getFallbackPrice(symbol: string): TokenPrice {
    ...
    logger.warn('BrowserDynamicPricingService', `Using fallback price for ${symbol}: $${price}`);
...
export default BrowserDynamicPricingService;
